...
class FindBookByIdQuery {
  ...
  public function query(string $id): Option {
    $stmt = $this->connection->prepare(
      "SELECT
                ...
                WHERE
                    B.ID = ?");
    $escapedId = $this->connection->real_escape_string($id);
    $stmt->bind_param("i", $escapedId);
    $stmt->execute();
    $result = $stmt->get_result();
    // TODO: create Book row mapper to reuse it in different queries
    //       or consider writing a findByWhereClause query and provide where clause dynamically?
